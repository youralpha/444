...
const PHASES_DEF = [
  { id: 'base20', title: 'База (20м)', icon: <Brain size={24} className="text-white" /> },
  { id: 'base30', title: 'База (30м)', icon: <Brain size={24} className="text-white" /> },
  ...
  { id: 'pmr', title: 'PMR', icon: <Wind size={24} className="text-green-500" /> },
  ...
  { id: 'act', title: 'ACT', icon: <Brain size={24} className="text-pink-400" /> },
  { id: 'rehearsal', title: 'Репетиция', icon: <Brain size={24} className="text-teal-400" /> },
  { id: 'cold', title: 'Холод', icon: <Wind size={24} className="text-cyan-400" /> },
  { id: 'key', title: 'Ключ', icon: <Brain size={24} className="text-yellow-400" /> },
];
...
export default function BaseTimer() {
  ...
  const initPhases = (id: string) => {
    let newPhases: Phase[] = [];
    if (id === 'hrv') newPhases = MODES.hrv(6.0);
  ...
  return (
    <div className="flex flex-col h-full fade-in pb-8">
      <div className="flex items-center gap-3 mb-8">
        <Timer className="text-green-500" size={28} />
      ...
      <div className="flex justify-center flex-wrap gap-2 mb-8 max-w-4xl mx-auto">
         {PHASES_DEF.map(p => (
           <button key={p.id} onClick={() => changePhase(p.id)} className={`flex items-center gap-1.5 px-3 py-2 border-2 rounded font-bold uppercase transition-all text-[10px] ${activePhaseId === p.id ? 'border-green-500 bg-green-500/10 text-white shadow-[0_0_10px_rgba(34,197,94,0.2)]' : 'border-tactical-700 text-tactical-text/50 hover:border-tactical-text/50'}`}>
             {p.icon} {p.title}
           </button>
         ))}
      </div>
      ...
      <div className="cyber-border flex flex-col items-center justify-center p-8 mb-8 relative">
        <div className="flex justify-between w-full mb-6 relative z-10">
           <button onClick={() => setInfoOpen(true)} className="text-tactical-text/50 hover:text-white transition-colors p-2 outline-none"><Info size={24} /></button>
           <button onClick={toggleRecord} className={`p-2 transition-colors rounded-full border-2 outline-none ${recording ? 'bg-red-500/20 border-red-500 text-red-500 animate-pulse' : 'border-tactical-700 text-tactical-text/50 hover:text-white hover:border-tactical-text/50'}`}><Mic size={24} /></button>
        </div>
        ...
        <div className="relative flex items-center justify-center mb-8">
            <svg width="300" height="300" className="rotate-[-90deg]">
                ...
                <circle cx="150" cy="150" r={radius} fill="transparent" stroke={curPhase?.color || "#22c55e"} strokeWidth="12"
                    strokeDasharray={circumference} strokeDashoffset={strokeDashoffset}
                    className="transition-all duration-100 ease-linear" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div className="text-xs font-bold uppercase text-tactical-text/60 mb-2 text-center px-4 leading-tight">{curPhase?.globalTitle || curPhase?.name}</div>
                <div className={`font-mono text-6xl font-bold tracking-tight ${running ? 'text-white drop-shadow-[0_0_15px_rgba(34,197,94,0.5)]' : 'text-tactical-text'}`}>
                    {formatTime(seconds)}
                </div>
            </div>
        </div>
        ...
        <input
          ...
          className="bg-tactical-900 border border-tactical-700 p-3 w-full max-w-md text-center rounded text-sm mb-8 focus:border-green-500 outline-none font-mono text-tactical-text/80 transition-colors relative z-10"
        />

        <div className="flex gap-4 relative z-10">
           <button onClick={toggleTimer} disabled={seconds <= 0} className={`flex items-center gap-2 px-8 py-3 rounded font-bold uppercase tracking-wider transition-all outline-none disabled:opacity-50 disabled:cursor-not-allowed ${running ? 'bg-orange-500/20 text-orange-400 border border-orange-500 hover:bg-orange-500/30' : 'bg-green-500 text-tactical-900 hover:bg-green-400 shadow-[0_0_15px_rgba(34,197,94,0.4)] hover:shadow-[0_0_25px_rgba(34,197,94,0.6)]'}`}>
             {running ? <><Pause size={18}/> Пауза</> : <><Play size={18}/> {seconds > 0 ? 'Старт' : 'Завершено'}</>}
           </button>
           <button onClick={resetTimer} className="flex items-center gap-2 px-6 py-3 rounded border border-tactical-700 text-tactical-text/50 hover:text-white hover:border-tactical-text/50 transition-colors outline-none uppercase font-bold text-sm">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={saveLog} className="text-xs bg-tactical-800 border border-tactical-700 px-3 py-1 rounded text-green-500 hover:bg-tactical-700 uppercase font-bold transition-colors outline-none">
          Сохранить в лог
        </button>
      </div>
          ...
          <tbody>
            ...
            {tasks.map(t => (
              <tr key={t.id} className="border-b border-tactical-800 hover:bg-tactical-800/50 transition-colors">
                ...
                <td className="p-3 text-xs font-mono text-green-500 font-bold text-right">{Math.floor(t.duration / 60)}м {t.duration % 60}с</td>
      ...
      <Dialog.Root open={infoOpen} onOpenChange={setInfoOpen}>
        <Dialog.Portal>
           ...
           <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100vw-3rem)] max-w-2xl bg-tactical-800 border border-tactical-700 rounded-sm shadow-2xl z-50 flex flex-col p-8 fade-in outline-none">
             ...
             <div className="flex flex-col gap-8 mb-6 max-h-[60vh] overflow-y-auto custom-scrollbar pr-4">
                 <p className="text-sm text-tactical-text/80">
                    Все техники скопированы из исходного <strong>BASE.html</strong>. Дополнительных docx файлов в репозитории не обнаружено, поэтому используется оригинальный текст приложения.
                 </p>
                {PHASES_DEF.map(p => (
                   <div key={p.id} className="bg-tactical-900 p-4 rounded border border-tactical-700">
                      <h3 className="text-sm font-bold text-green-500 flex items-center gap-2 mb-2 uppercase">{p.icon} {p.title}</h3>
                      <p className="text-sm text-white font-mono mb-2">Нажмите на кнопку техники на главном экране, чтобы запустить автоматическое сопровождение фаз.</p>
                   </div>
                ))}
             </div>
             <div className="flex justify-end">
                <button onClick={() => setInfoOpen(false)} className="px-6 py-2 bg-green-500 text-tactical-900 font-bold text-sm uppercase rounded hover:bg-green-400 transition-colors outline-none">Понятно</button>
             </div>
           </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
