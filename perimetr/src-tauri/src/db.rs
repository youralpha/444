    ...
    fn init_schema(&self) -> Result<()> {
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS kpt_state (
                id INTEGER PRIMARY KEY, xp INTEGER DEFAULT 0, level INTEGER DEFAULT 1, current_week INTEGER DEFAULT 1
            )", [])?;

        conn.execute(
            "CREATE TABLE IF NOT EXISTS kpt_tasks (
                task_id TEXT PRIMARY KEY, completed BOOLEAN
            )", [])?;

        conn.execute(
            "CREATE TABLE IF NOT EXISTS kpt_inputs (
                input_id TEXT PRIMARY KEY, value TEXT
            )", [])?;

        // Calendar
        conn.execute(
            "CREATE TABLE IF NOT EXISTS kpt_calendar (
                date TEXT PRIMARY KEY, has_activity BOOLEAN
            )", [])?;
